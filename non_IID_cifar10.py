...
def load_data(delta):
    ...
    # Create 10 groups, each only containing one class
    for class_id in range(10):
        class_indices = np.where(targets == class_id)[0]
        np.random.shuffle(class_indices)
        original_groups.append(class_indices)

    client_datasets = []
    group_size = len(original_groups[0])  # Each class has 5000 samples
    shared_per_group = int(group_size * delta / 10)

    for client_id in range(10):
        shared_data = []

        # Shared data from all groups
        for group in original_groups:
            shared_data.extend(group[:shared_per_group])

        # Trim shared data to a max of 6000 samples
        shared_data = shared_data[:6000]

        # Add private data from the client's own group if needed
...
def federated_learning(model_type, rounds, delta):
    ...
    filename = f"result2/non_IID_cifar10_{model_type}_{delta}.csv"
