...
def main():
    if len(sys.argv) != 3:
        print("Usage: python plot_IID.py <dataset> <model>")
        sys.exit(1)
    ...
    for delta in deltas:
        file_path = os.path.join(result_dir, f"IID_{dataset}_{model}_{delta}.csv")
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(f"{dataset}_plot/IID_{model}.png")
