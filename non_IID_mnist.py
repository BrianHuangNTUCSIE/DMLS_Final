...
class CNN(nn.Module):
    def __init__(self):
        ...
        self.conv = nn.Sequential(
            nn.Conv2d(1, 6, kernel_size=5, stride=1, padding=2),  # (N, 1, 28, 28) -> (N, 6, 28, 28)
            nn.Tanh(),
            nn.AvgPool2d(kernel_size=2, stride=2),                # (N, 6, 14, 14)
            nn.Conv2d(6, 16, kernel_size=5, stride=1),            # (N, 16, 10, 10)
            nn.Tanh(),
...
def get_model(model_type):
    if model_type == "LR":
        ...
        # Remove the initial maxpooling layer for small images like MNIST
...
def load_data(delta):
    ...
    # Step 2: Create original groups where each group only contains one class
    original_groups = []
    group_size = len(train_dataset) // 10
    for class_id in range(10):
        indices = class_to_indices[class_id][:group_size]  # Ensure equal group size
        original_groups.append(indices)

    # Step 3: Build client datasets
    client_datasets = []
    shared_per_group = int(group_size * delta / 10)
    for client_id in range(10):
        shared_data = []

        # Collect shared data from all 10 groups
        for group in original_groups:
            shared_data.extend(group[:shared_per_group])
        shared_data = shared_data[:6000]  # Cap at 6000

        # Fill in remaining with private data from client's own group
        if len(shared_data) < 6000:
            own_group = original_groups[client_id]
            used_shared = set(shared_data)
            private_candidates = [idx for idx in own_group if idx not in used_shared]
            shared_data.extend(private_candidates[:6000 - len(shared_data)])
...
def federated_learning(model_type, rounds, delta):
    ...
    filename = f"result2/non_IID_mnist_{model_type}_{delta}.csv"
