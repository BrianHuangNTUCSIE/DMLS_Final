...
# ----- Model Definitions for CIFAR-100 -----
class LR(nn.Module):
    def __init__(self):
        super(LR, self).__init__()
        self.linear = nn.Linear(3 * 32 * 32, 100)  # Change to 100 classes
...
class DNN(nn.Module):
    def __init__(self):
        ...
        self.net = nn.Sequential(
            ...
            nn.Linear(200, 200),
            nn.ReLU(),
            nn.Linear(200, 100)  # Change to 100 classes
        )
...
class CNN(nn.Module):
    def __init__(self):
        ...
        self.fc = nn.Sequential(
            ...
            nn.Linear(120, 84),
            nn.Tanh(),
            nn.Linear(84, 100)  # Change to 100 classes
        )
...
def get_model(model_type):
    if model_type == "LR":
        ...
        model = models.densenet121(num_classes=100)  # Change to 100 classes
        return model
    else:
        raise ValueError("Unknown model type. Choose from: LR, DNN, CNN, DenseNet.")

# ----- Load and Split CIFAR-100 -----
def load_data(delta):
    ...
    class_to_indices = {i: [] for i in range(100)}
    for idx, (_, label) in enumerate(train_dataset):
        class_to_indices[label].append(idx)

    # Build original groups: each group has 10 consecutive classes
    original_groups = []
    for group_id in range(10):
        group_indices = []
        for class_id in range(group_id * 10, (group_id + 1) * 10):
            group_indices.extend(class_to_indices[class_id])
        np.random.shuffle(group_indices)
        original_groups.append(group_indices)

    client_datasets = []
    group_size = min(len(group) for group in original_groups)
    ...
    for client_id in range(10):
        ...
        shared_data = shared_data[:6000]  # Total shared samples per client
...
def federated_learning(model_type, rounds, delta):
    ...
    filename = f"result2/non_IID_cifar100_{model_type}_{delta}.csv"  # Update filename
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Federated Learning with CIFAR-100")
